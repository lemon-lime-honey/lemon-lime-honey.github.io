...
- **배포 파이프라인 업데이트**: `.github/workflows/deploy.yml`을 수정하여 GitHub Actions가 Node.js 26 환경에서 빌드되도록 명시하고, 재현성 및 버전 안정성을 보장하기 위해 `package-manager`를 `pnpm@latest`에서 `package.json`과 동일한 `pnpm@10.28.1`로 핀 고정 완료.
