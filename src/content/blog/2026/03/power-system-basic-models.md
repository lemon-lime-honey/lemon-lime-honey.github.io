...
- 추후 단락 사고 계산 시 기여도를 분리하기 위해 일반 부하와 전동기 부하를 구분하는 `LoadType` 열거형(`General`, `Motor`)을 추가했다.
- `Line` 클래스는 송전 선로 분포 정수 파라미터(저항, 인덕턴스, 컨덕턴스, 커패시턴스)를 입력받아 교류 임피던스와 어드미턴스를 도출하도록 모델링했다.
- 주파수 종속 리액턴스와 서셉턴스를 계산하기 위해 C++20 표준 상수 `std::numbers::pi`를 적용했다.

## 초기 토폴로지 구성을 위한 모선 객체 설계

- 전력망 내 위상수학적 연결 거점 역할을 수행할 `Bus` 객체를 설계해 고유 식별자(`id`)와 공칭 전압(`nominalVoltage`) 데이터를 관리하도록 구성했다.
- 초기 아키텍처 단계에서는 개별 `Bus` 객체가 직접 `std::vector<std::shared_ptr<models::Load>>` 및 `Line` 포인터 컨테이너(`loads_`, `lines_`)를 소유하여 인접 하위 컴포넌트 위상 연결을 관리하는 방식을 적용했다.
