...
고장 계산 알고리즘 엔진과 보호 협조 모듈을 end-to-end 파이프라인으로 통합했다. 모듈 간 연결 시 발생할 수 있는 에러를 막기 위해 방어적 예외 처리 로직을 적용했다.
...
- CT 매핑이 누락되었거나 리포트 내 선로 데이터가 없을 경우 `std::invalid_argument` 및 `std::runtime_error`를 명시적으로 던지는 Fail-Fast 예외 처리 구조를 구현했다.
- `ProtectionCoordinatorTest`에 모의 선로 및 CT 매핑 환경을 구축하고, 영상 전류 기반 `OCGR` 계전기가 정상적으로 차단기를 트립시키는 단위 테스트를 작성했다.
