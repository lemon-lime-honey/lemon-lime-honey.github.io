...
전력망 토폴로지를 수식으로 표현하는 어드미턴스 행렬(Y-Bus)을 동적으로 구성할 목적으로 `math` 및 `analysis` 스코프 내 핵심 유틸리티를 개발했다. 외부 의존성 없이 독립 구동이 가능한 복소수 기반 선형대수 연산 코드를 구축했다.
...
- `YBusBuilderTest` 및 `ComplexMatrixTest` 단위 테스트를 작성해 역행렬 연산 시 발생하는 특이 행렬 오류 판별과 복소수 연산 수치 정확성을 엄격히 검증했다.
