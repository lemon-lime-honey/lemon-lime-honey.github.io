...
수집된 방대한 시계열 데이터를 바탕으로 차량 평균/최대 속도, 급가/감속 횟수, 브레이크 조작 횟수, 객체 감지 빈도 등 주행 통계 리포트를 산출하는 집계 시스템을 구축했다. 그러나 여러 클라이언트가 동시다발적으로 분석을 요청할 때 애플리케이션 계층에 심각한 메모리 부하가 유발되었다. 이를 해결하기 위해 데이터베이스 엔진 단에서 대규모 연산을 직접 수행하도록 아키텍처를 전면 재설계했다.
